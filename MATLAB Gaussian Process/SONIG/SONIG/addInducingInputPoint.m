...
% 	Kpp = sonig.hyp.ly(i)^2*exp(-1/2*sum((diff1./repmat(permute(sonig.hyp.lx(:,i),[2,3,1]),np,np,1)).^2,3));
    Kpp = sonig.hyp.ly(i)^2*exp(-1/2*sum((diff1./repmat(permute(sonig.hyp.lx(:),[2,3,1]),np,np,1)).^2,3));
	%Kup = sonig.hyp.ly(i)^2*exp(-1/2*sum((diff2./repmat(permute(sonig.hyp.lx(:,i),[2,3,1]),sonig.nu,np,1)).^2,3));
    Kup = sonig.hyp.ly(i)^2*exp(-1/2*sum((diff2./repmat(permute(sonig.hyp.lx(:),[2,3,1]),sonig.nu,np,1)).^2,3));
	Kpu = Kup';
