...
while counter < 30 && (exist('prevxl','var') == 0 || sum(((xl-prevxl)./smallestLengthScales).^2) > 0.001) % We continue iterating until the change between xl and the previous xl (when normalized) is below a certain threshold. We also have a security check on the maximum number of iterations.
	...
	for i = 1:sonig.dy
		...
		%lengthDivisor = repmat(permute(sonig.hyp.lx(:),[3,2,1]),[1,sonig.nu,1]);
        lengthDivisor = repmat(permute(sonig.hyp.lx(:),[3,2,1]),[1,sonig.nu,1]);
...
% 	lengthDivisor = repmat(permute(sonig.hyp.lx(:),[3,2,1]),[1,sonig.nu,1]);
	...
	d2Kxu(:,:,:,:,i) = repmat(Kxu(:,:,:,:,i),[1,1,sonig.dx,sonig.dx]).*(mmat(diffNormalized2, permute(diffNormalized2, [1,2,4,3]), [3,4]) - repmat(permute(diag(1./sonig.hyp.lx(:).^2),[3,4,1,2]),[1,sonig.nu,1,1]));
