...
for i = 1:sonig.dy
	...
	Ktt = sonig.hyp.ly(i)^2*exp(-1/2*sum((diff1./repmat(permute(sonig.hyp.lx(:),[2,3,1]),[nt,nt,1])).^2,3));
	Kut = sonig.hyp.ly(i)^2*exp(-1/2*sum((diff2./repmat(permute(sonig.hyp.lx(:),[2,3,1]),[sonig.nu,nt,1])).^2,3));
	Ktu = Kut';
	...
	if nargout >= 2
       ...
       % postCov(:,:,i) = Ktt - Ktu/sonig.Kuu{i}*(sonig.Kuu{i} - eye(size(sonig.Kuu{i}))*i*0.001)/sonig.Kuu{i}*Kut;
	end
	if nargout >= 3
